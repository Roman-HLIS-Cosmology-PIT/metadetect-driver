...
def _main(input_dir, output_dir, truth_dir, mosaic):
    ...
    detection_table = detection_dataset.to_table(filter=quality)
    ...
    axs.scatter(
        detection_xs[(detection_xs > 0) & (detection_ys > 0) & (detection_xs < ncol) & (detection_ys < nrow)],
        detection_ys[(detection_xs > 0) & (detection_ys > 0) & (detection_xs < ncol) & (detection_ys < nrow)],
        c='k',
        marker='x',
        s=72,
        label='Detection',
    ...
    # dataset_quality = dataset.filter(quality)
    # dataset_nquality = dataset.filter(~quality)

    print(f"Found {detection_table.num_rows} detections")

    plt.hist2d(
        detection_table["gauss_s2n"],
        detection_table["gauss_T_ratio"],
        bins=[
            np.geomspace(1, 1_000, 101),
    ...
    plt.hist2d(
        -2.5 * np.log10(
            pc.divide(
                pc.list_element(detection_table["pgauss_band_flux"], 0),
                pc.list_element(detection_table["pgauss_band_flux"], 1),
            )
        ),
        -2.5 * np.log10(
            pc.divide(
                pc.list_element(detection_table["pgauss_band_flux"], 1),
                pc.list_element(detection_table["pgauss_band_flux"], 2),
            )
        ),
        bins=[
            np.linspace(-0.5, 1.0, 101),
            np.linspace(-0.5, 1.0, 101),
        ],
        norm="log",
        rasterized=True,
    ...
    for i, band in enumerate(BANDS):
        ...
        axs[i].hist2d(
            -2.5 * np.log10(galaxy_table.take(detection_galaxy_match.indices_to_second_set)[f"roman_flux_{ROMAN_BAND_KEYS[band]}"]) + ROMAN_BANDPASSES[ROMAN_BAND_KEYS[band]].zeropoint,
            -2.5 * np.log10(pc.list_element(detection_table.take(detection_galaxy_match.indices_to_first_set)["pgauss_band_flux"], i)) + ROMAN_BANDPASSES[ROMAN_BAND_KEYS[band]].zeropoint,
            norm="log",
            bins=bins,
            rasterized=True,
        )
    ...
    for i, band in enumerate(BANDS):
        ...
        axs[i].hist(
            -2.5 * np.log10(pc.list_element(detection_table["pgauss_band_flux"], i)) + ROMAN_BANDPASSES[ROMAN_BAND_KEYS[band]].zeropoint,
            bins=bins,
            histtype="step",
            ec="k",
            label="Detections",
