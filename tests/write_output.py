...
def _write_catalogs(catalogs, base_dir):
    ...
    for shear_type in shear_types:
        ...
        print(f"Opening parquet writer for {shear_type} at {output_file}")
    ...
    for shear_type in shear_types:
        print(f"Closing parquet writer for {shear_type}")
...
def main():
    ...
    driver_config = deepcopy(metadetect_driver.defaults.DRIVER_DEFAULTS)
    driver_config["mdet_seed"] = 42

    results = metadetect_driver.run_metadetect(outimages, driver_config=driver_config)
